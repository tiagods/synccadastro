...
public class WorkbookServices {
    ...
    public List<Cliente> readWorkbook(String caminho) throws IOException {
        ...
        Biff8EncryptionKey.setCurrentUserPassword(senha);
        ...
        while (linha.hasNext()) {
            ...
            if (row.getRowNum() == 0) {
                ...
                while (columns.hasNext()) {
                    ...
                    if (valor.toUpperCase().trim().equals("COD"))
                        positionCodigo = celula.getColumnIndex();
                }
            ...
            while (columns.hasNext()) {
                ...
                if (row.getRowNum() != 0 && celula.getColumnIndex() == positionCodigo) {
                    try {
                        Integer.parseInt(readingCell(celula));// se nao conseguir o cast, o campo codigo
    ...
    public void removeTempWorkbook(String diretorio){
        try{
            //FileSystemUtils.deleteRecursively(diretorio);
